...
void solve() {
    ...
    rep(i,0,n-1)
    {
        if(s[i]==s[i+1] && s[i]=='0') {cout<<-1<<endl;return;}
    }
    int sum;
    if(s[0]=='+') sum=1;
    else sum=-1;
    int ans=abs(sum);
    if(s.find("-++-")!=string::npos || s.find("+--+")!=string::npos) {cout<<3<<endl;return;}
    rep(i,1,n)
    {
        if(s[i]=='0')
        {
            if(abs(sum) !=1) {cout<<2<<endl;return;}
            sum=0;
        }
        if(s[i]=='+')
        {
            if(sum==0) {sum++;}
            else if(sum<0) {cout<<2<<endl;return;}
            else if(sum>0) {
                if(sum>1) sum--;
                else sum++;
            }
