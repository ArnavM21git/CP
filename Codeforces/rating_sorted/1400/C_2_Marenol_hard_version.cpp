...
void solve() {
    ...
    if(c1a!=c1b) {cout<<-1<<endl;return;}
    if(c0a!=c0b) {cout<<-1<<endl;return;}
    ...
    if(c0aa!=c0bb) {cout<<-1<<endl;return;}

    vi idxea,idxeb,idxoa,idxob;
    rep(i,0,n)
    {
        if(a[i]=='1')
        {
    ...
    rep(i,0,idxea.size())
    {
        ans+=abs(idxea[i]-idxeb[i])/2;
    }
    rep(i,0,idxoa.size())
    {
        ans+=abs(idxoa[i]-idxob[i])/2;
    }
    cout<<ans<<endl;
